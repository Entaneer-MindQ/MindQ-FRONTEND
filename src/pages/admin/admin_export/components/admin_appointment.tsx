...
const Admin_Test: React.FC = () => {
  const token = document.cookie;// useCookies(["auth_token"]);
  ...
  const downloadCSV = async () => {
    ...
    try {
      const response = await fetch(`${BASE_URL}${endpoint}`, {
        ...
        // body: JSON.stringify({ filter: "someValue" }), // Example request payload
        body: JSON.stringify({}),
      });

      if (!response.status) throw new Error("Failed to download CSV");
